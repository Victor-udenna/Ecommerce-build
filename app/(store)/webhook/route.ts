...
export async function POST(req: NextRequest) {
  ...
  //   signature is to verity using the webhook secret if is came from stripe
...
async function createOrderInSanity(session: Stripe.Checkout.Session) {
  const { id, amount_total, currency, customer_details, metadata, payment_intent, customer, total_details } = session;
  ...
  const order = await backendClient.create({
    ...
    customerName: customer_details?.name ?? customerName,
    stripeCustomerId: customer,
    clerkUserId: clerkUserId,
    email: customer_details?.email ?? customerEmail,
    currency,
